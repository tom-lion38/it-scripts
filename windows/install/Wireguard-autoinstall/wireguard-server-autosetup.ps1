# wireguard-server-autosetup.ps1 (v7 - WinNAT error fixed + split-tunnel fallback)
...
$pick = if ($ifcs) { Select-FromList "Choisis l'interface WAN (celle qui sort Internet/LAN principal)" $ifcs { param($x) "$($x.InterfaceAlias)  |  $($x.IPv4Address.IPAddress)" } } else { $null }
...
if ($routeMode -eq "2") { $clientAllowed = Read-Default "AllowedIPs (séparés par virgules)" "$cidr,192.168.1.0/24" }
...
if ($doNat) {
  ...
  if (-not $okNat) {
    Warn "WinNAT est indisponible sur ta machine (0x80041013)."
    Warn "=> Solution simple: passe en SPLIT-TUNNEL (AllowedIPs = LAN + WG), pas besoin de NAT."
    if (Read-YesNo "Basculer automatiquement en split-tunnel (LAN seulement) ?" "y") {
      $clientAllowed = Read-Default "AllowedIPs (split-tunnel)" "$cidr,192.168.1.0/24"
      Warn "OK. Tes clients n'auront PAS internet via ce serveur, seulement les réseaux choisis."
    } else {
      Warn "Si tu veux FULL-TUNNEL sur Windows Server: utilise RRAS (Remote Access -> NAT)."
    }
  }
}
